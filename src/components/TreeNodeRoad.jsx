...
export default function TreeNode({
  ...
  const hasDivisions = node.divisions && Object.keys(node.divisions).length > 0;
  const hasProvisions = node.provisions && node.provisions.length > 0;
  const hasChildren = hasDivisions || hasProvisions;
  ...
  function isNodeOrDescendantMatched(node, results) {
    ...
    if (node.divisions) {
      return Object.values(node.divisions).some((division) => isNodeOrDescendantMatched(division, results));
    }
    if (node.provisions) {
  ...
  return (
    <div style={indent} className="text-left">
      ...
      {open && hasDivisions &&
        Object.values(node.divisions).map((division) => (
          <TreeNode
            key={division.id}
            node={division}
            level={level + 1}
