...
export default function Climate() {
    ...
    const load = async () => {
      try {
        ...
        const [ncrRes, sinRes] = await Promise.all([
          fetch(`http://115.146.87.57:8000/paragraphs/ncr`),
          fetch(`http://115.146.87.57:8000/paragraphs/singapore`)
        ]);
        ...
        const [ncrCntRes, sinCntRes] = await Promise.all([
          fetch(`http://115.146.87.57:8000/pair_counts/ncr`),
          fetch(`http://115.146.87.57:8000/pair_counts/singapore`)
        ]);
  ...
  const handleClick = async (side, paraObj) => {
    try {
      ...
      const res = await fetch(`http://115.146.87.57:8000/pairs?doc=${docName}&para_id=${paraObj.para_id}&size=1000`);
